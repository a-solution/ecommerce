...
class ControllerCatalogFaq extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_faq->addReview($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_faq->editReview($this->request->get['review_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $review_id) {
				$this->model_catalog_faq->deleteReview($review_id);
			}
	...
	protected function getList() {
		...
		$review_total = $this->model_catalog_faq->getTotalReviews($filter_data);

		$results = $this->model_catalog_faq->getReviews($filter_data);
		...
		foreach ($results as $result) {
			$data['reviews'][] = array(
				...
				'edit'       => $this->url->link('catalog/faq/edit', 'token=' . $this->session->data['token'] . '&review_id=' . $result['review_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_product'] = $this->url->link('catalog/faq', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
		$data['sort_author'] = $this->url->link('catalog/faq', 'token=' . $this->session->data['token'] . '&sort=r.author' . $url, 'SSL');
		$data['sort_rating'] = $this->url->link('catalog/faq', 'token=' . $this->session->data['token'] . '&sort=r.rating' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('catalog/faq', 'token=' . $this->session->data['token'] . '&sort=r.status' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('catalog/faq', 'token=' . $this->session->data['token'] . '&sort=r.date_added' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['review_id'])) {
			...
			$data['action'] = $this->url->link('catalog/faq/edit', 'token=' . $this->session->data['token'] . '&review_id=' . $this->request->get['review_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('catalog/faq', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['review_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$review_info = $this->model_catalog_faq->getReview($this->request->get['review_id']);
		}
