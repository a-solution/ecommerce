...
class ModelCatalogFaq extends Model {
    ...
    public function addFaq($product_id, $data) {
        $this->event->trigger('pre.faq.add', $data);

        $this->db->query("INSERT INTO " . DB_PREFIX . "faq SET author = '" . $this->db->escape($data['name']) . "', customer_id = '" . (int) $this->customer->getId() . "', product_id = '" . (int) $product_id . "', text = '" . $this->db->escape($data['text']) . "', rating = '" . (int) $data['rating'] . "', date_added = NOW()");
        ...
        $this->event->trigger('post.faq.add', $review_id);
    }
    ...
    public function getFaqsByProductId($product_id, $start = 0, $limit = 20) {
        ...
        $query = $this->db->query("SELECT r.review_id, r.author, r.rating, r.text, r.answer, r.answer_text, p.product_id, pd.name, p.price, p.image, r.date_added FROM " . DB_PREFIX . "faq r LEFT JOIN " . DB_PREFIX . "product p ON (r.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int) $product_id . "' AND p.date_available <= NOW() AND p.status = '1' AND r.status = '1' AND pd.language_id = '" . (int) $this->config->get('config_language_id') . "' ORDER BY r.date_added DESC LIMIT " . (int) $start . "," . (int) $limit);

        return $query->rows;
    }

    public function getTotalFaqsByProductId($product_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "faq r LEFT JOIN " . DB_PREFIX . "product p ON (r.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int) $product_id . "' AND p.date_available <= NOW() AND p.status = '1' AND r.status = '1' AND pd.language_id = '" . (int) $this->config->get('config_language_id') . "'");
