...
class ModelCatalogProduct extends Model {
    ...
    public function getProduct($product_id, $include_category = false) {
        $queryString = ("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer,"
                ...
                . " , (SELECT SUM(orp.quantity) FROM " . DB_PREFIX . "order_product orp WHERE orp.product_id = p.product_id) AS 'purchased' "
        ...
        if ($query->num_rows) {
            return array(
                ...
                'purchased' => ($query->row['purchased'] == null) ? 0 : (int) $query->row['purchased']
            );
        } else {
            return false;
        }
    ...
    public function getProducts($data = array()) {
        ...
        //if(isset($data['sort']) && $data['sort'] == 'purchased'){
        ...
        foreach ($query->rows as $result) {
            ...
            $product_data[$result['product_id']] = $theProduct;
        }
    ...
    public function getBestSellerProducts($limit, $category_ids = '') {

        $cache_key = 'product.bestseller.' . (int) $this->config->get('config_language_id') . '.' . (int) $this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id') . '.' . (int) $limit;
        if ($category_ids != null && $category_ids != '') {
            $arr_cate_ids = explode(",", $category_ids);
            sort($arr_cate_ids);
            $cache_key .= ('.' . implode('_', $arr_cate_ids));
        }
    ...
    public function getProductCategoryPath ($product_id) {
        $query = $this->db->query("SELECT GROUP_CONCAT(path_id ORDER BY LEVEL ASC SEPARATOR '_') as path FROM " . DB_PREFIX . "product_to_category LEFT JOIN " . DB_PREFIX . "category_path USING (category_id) WHERE product_id = '" . (int)$product_id . "' AND category_id = (SELECT category_id FROM " . DB_PREFIX . "product_to_category c LEFT JOIN " . DB_PREFIX . "category_path cp USING (category_id) WHERE product_id = '" . (int)$product_id . "' AND cp.level = (SELECT max(LEVEL) FROM " . DB_PREFIX . "product_to_category LEFT JOIN " . DB_PREFIX . "category_path USING (category_id) WHERE product_id = '" . (int)$product_id . "') ORDER BY category_id LIMIT 1) GROUP BY category_id");
