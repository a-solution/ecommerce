...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		foreach ($data['products'] as $product) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");                        
                        ...
                        //Hieu: Subtract product quantity if ordered
			...
			foreach ($product['option'] as $option) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', name = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "'");
                                ...
                                //Hieu: Subtract product options if ordered
	...
	public function editOrder($order_id, $data, $order_info = null) {
		$this->event->trigger('pre.order.edit', $data);

		// Void the order first
		$this->addOrderHistory($order_id, 0);

                //If changing status to CANCELED, restock product quantity and options
                if($order_info == null){
                    $order_info = $this->getOrder($order_id);    
                }

                $needRestock = $order_info && isset($data['order_status_id']) && ($data['order_status_id'] != $order_info['order_status_id']) && $data['order_status_id'] == '7'; //Complete - 5, Canceled - 7
	...
	public function deleteOrder($order_id, $order_info = null) {
		$this->event->trigger('pre.order.delete', $order_id);

		// Void the order first
		$this->addOrderHistory($order_id, 0);
                
                //If delete UN-COMPLETED order, do restock product quantity and some product options
                //If the order was CANCELED before that, don't restock, it was restocked right after CANCALED
                if($order_info == null){
                    $order_info = $this->getOrder($order_id);    
                }
                
                $needRestock = $order_info && !in_array($order_info['order_status_id'], ['5', '7']); //Complete - 5, Canceled - 7
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		$this->event->trigger('pre.order.history.add', $order_id);
                ...
                //Hieu: Should optimize to get order info only once
		$order_info = $this->getOrder($order_id);
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) 
                                ...
                                //Hieu: Why subtract product quantity ??? Subtraction had been DONE after ordered
                                // Stock subtraction
				/*
                                $order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) 
                                ...
                                //Hieu: NO Restock !!! It should be DONE when DELETE order
                                // Restock
				/*
                                $product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");
